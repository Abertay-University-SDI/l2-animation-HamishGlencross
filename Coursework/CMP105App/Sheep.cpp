...
Sheep::Sheep()
{
	for (int i = 0; i < 4; i++) 
	{
		Sheep::m_walkDown.addFrame(sf::IntRect({ i * 64, 0 }, { 64, 64 }));
	}
	...
	for (int i = 5; i < 8; i++)
	{
		Sheep::m_walkUp.addFrame(sf::IntRect({ i * 64, 0 }, { 64, 64 }));
	}
	m_walkUp.setLooping(true);
	m_walkUp.setFrameSpeed(1.f / 4.f);
	for (int i = 0; i < 4; i++)
	{
		Sheep::m_walkUpRight.addFrame(sf::IntRect({ i * 64, 64 }, { 64, 64 }));
	}
	m_walkUpRight.setLooping(true);
	m_walkUpRight.setFrameSpeed(1.f / 4.f);
	for (int i = 5; i < 8; i++)
	{
		Sheep::m_walkRight.addFrame(sf::IntRect({ i * 64, 64 }, { 64, 64 }));
	}
	m_walkRight.setLooping(true);
	m_walkRight.setFrameSpeed(1.f / 4.f);
	for (int i = 0; i < 4; i++)
	{
		Sheep::m_walkDownRight.addFrame(sf::IntRect({ i * 64, 128}, { 64, 64 }));
	}
...
void Sheep::handleInput(float dt)
{
	...
	else if (m_input->isKeyDown(sf::Keyboard::Scancode::Space))
	{
